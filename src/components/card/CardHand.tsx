...
const CardHandSlot = ({i, id, angle, unitCircleLength}: {i: number, id: slotId, angle: number, unitCircleLength: number}) => {
    // const offsetY = 200 * Math.sin(Math.abs(angle / 180 * Math.PI));
    const offsetY = unitCircleLength * (1 - Math.cos(angle / 180 * Math.PI));
    return (
        <div className="w-24 flex-shrink-[10] min-w-0" style={{paddingTop: `${offsetY}px`}}>
...
const CardHand = (props: HTMLAttributes<HTMLDivElement>) => {
    ...
    const cardSlotPositions = useSelector((state: RootState) => state.cards.slotPositions);

    const numCardSlots = Object.keys(cardSlots).length;
    const maxCardAngle = lerp(3, maxCardAngleFullHand, cards.length / 10);
    const minCardX = Object.values(cardSlotPositions).reduce((prev, curr) => Math.min(prev, curr.x), 0);
    const maxCardX = Object.values(cardSlotPositions).reduce((prev, curr) => Math.max(prev, curr.x), 0);
    const unitCircleLength = (maxCardX - minCardX) / (2 * Math.sin(maxCardAngle / 180 * Math.PI));
    return (
        <>
            <div className="flex flex-row justify-center w-full px-16" {...props}>
                {Object.values(cardSlots).map((slotId, i) => 
                    <CardHandSlot key={elementId(slotId)}
                                id={slotId}
                                i={i}
                                unitCircleLength={unitCircleLength}
                                angle={lerp(-maxCardAngle, maxCardAngle, i / (numCardSlots - 1))} />)}
            </div>
            {cards.map((card, i) => <CardRenderer canDrag={true} key={card.id} slotId={cardSlots[card.id]} card={card}></CardRenderer>)}
        </>
    )
}
